...
>(
    ...
    if let Some(target_gui) = target_gui {
        match target_gui.as_str() {
            "batch" => {
                return batch::BatchGui::go(pause, f)
                    .unwrap_or(ExitCode::FAILURE)
            }
            #[cfg(target_os = "macos")]
            "cocoa" => {
                return cocoa::CocoaGui::go(pause, f)
                    .unwrap_or(ExitCode::FAILURE)
            }
            #[cfg(feature = "gui_liso")]
            "liso" => {
                return liso::LisoGui::go(pause, f)
                    .unwrap_or(ExitCode::FAILURE)
            }
            _ => {
                eprintln!(
                    "The GUI type you requested is unknown or unavailable. Try \"--gui help\"."
                );
