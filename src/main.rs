...
impl Cat {
    fn try_parse<'a>(
        ...
        let size = u64::from_be_bytes(
            bytes[newline + 33..newline + 41].try_into().unwrap(),
        );
        let xt = u16::from_be_bytes(
            bytes[newline + 41..newline + 43].try_into().unwrap(),
        );
        let next = newline + 43 + xt as usize;
        if next > bytes.len() {
            return Err(());
        }
        if is_fishy_path(file_path) {
            return Err(());
        }
...
fn try_load_url_from_file(
    ...
    if verbose {
        gui.borrow_mut()
            .verbose(&format!("Looking for update URL in: {:?}", path));
    }
    ...
    for line in f.lines() {
        ...
        if line.starts_with("URL=") {
            ...
            let url = match Url::parse(slab) {
                ...
                Err(_) => {
                    if verbose {
                        gui.borrow_mut().verbose(&format!(
                            "  File exists, but its URL= line does not contain a valid URL"
                        ));
                    }
                    return None;
                }
    ...
    if verbose {
        gui.borrow_mut()
            .verbose(&format!("  File exists, but has no URL= line"));
    }
    None
}
...
fn find_target_url(
    ...
    if target_url == None {
        ...
        target_url =
            try_load_url_from_file(&gui, verbose, Path::new(CONFIG_FILE_PATH));
    }
    ...
    match target_url.scheme() {
        ...
        x => {
            eprintln!(
                "{:?} is not a supported URL scheme. Only http and https are supported.",
                x
            );
            return Err(());
        }
...
async fn determine_tasks(
    gui: &Rc<RefCell<dyn Gui>>,
    verbose: bool,
    client: &mut reqwest::Client,
    target_url: &Url,
) -> Result<(Vec<Cat>, Vec<PathBuf>), ()> {
    gui.borrow_mut()
        .set_progress("Downloading update index...", "", None);
    let body = match client.get(target_url.clone()).send().await {
        Ok(x) if x.status() == 200 => x.bytes().await.unwrap(),
        Ok(x) => {
            gui.borrow_mut().do_error(
                "Download failed",
                &format!(
                    "Error \"{}\" while trying to download the update index.",
                    x.status()
                ),
            );
            return Err(());
        ...
        Err(x) => {
            gui.borrow_mut().do_error(
                "Download failed",
                &format!(
                    "Couldn't download the update index. The error was:\n{}",
                    x
                ),
            );
            return Err(());
    ...
    gui.borrow_mut()
        .set_progress("Determining files to update...", "", None);
    let (installs, deletes) = match find_updates(
        gui.clone(),
        verbose,
        &body[..],
        target_url.clone(),
            ...
            for path in glob.walk(&base) {
                let path = match path {
                    ...
                    Err(x) => {
                        if let Some(x) = x
                            .source()
                            .and_then(|x| x.downcast_ref::<std::io::Error>())
                        {
    ...
    all_deletions.sort_by(|a, b| a.path().cmp(&b.path()));
    ...
    for (n, (basedir, caturl)) in installs.iter().enumerate() {
        if patience.have_been_patient() {
            gui.borrow_mut().set_progress(
                "Downloading update catalogs...",
                &format!("{}/{} {}", n + 1, installs.len(), caturl),
                Some(n as f32 / installs.len() as f32),
            );
        }
        let body = match client.get(caturl.clone()).send().await {
            ...
            Ok(x) => {
                gui.borrow_mut().do_error(
                    "Download failed",
                    &format!(
                        "Error \"{}\" while trying to download an update catalog.",
                        x.status()
                    ),
                );
                return Err(());
            ...
            Err(x) => {
                gui.borrow_mut().do_error(
                    "Download failed",
                    &format!("Couldn't download an update catalog. The error was:\n{}", x),
                );
                return Err(());
            }
        };
        if body.len() == 0 {
            if verbose {
                gui.borrow_mut()
                    .verbose(&format!("{}: empty cat body", caturl));
            }
        ...
        if &body[..5] != b"\xFFTCat" {
            if verbose {
                gui.borrow_mut()
                    .verbose(&format!("{}: invalid cat header", caturl));
            }
        ...
        let uncompressed_size =
            u32::from_be_bytes(body[37..41].try_into().unwrap()) as usize;
        let mut uncompressed = Vec::with_capacity(uncompressed_size as usize);
        let mut reader = flate2::read::ZlibDecoder::new(&body[41..]);
        if reader.read_to_end(&mut uncompressed).is_err()
            || uncompressed.len() != uncompressed_size
            || lsx::sha256::hash(&uncompressed) != checksum
        {
            if verbose {
                gui.borrow_mut()
                    .verbose(&format!("{}: failed decompression", caturl));
            }
            gui.borrow_mut().do_error("Invalid catalog", &format!("A catalog file was invalid. This is a problem with the update server. Try again in a few minutes.\nThe corrupted catalog is: {}", caturl));
            return Err(());
        }
    ...
    Ok((
        all_cats,
        all_deletions.into_iter().map(|x| x.into_path()).collect(),
    ))
}

fn find_cat_statuses(
    gui: &Rc<RefCell<dyn Gui>>,
    verbose: bool,
    all_cats: &mut Vec<Cat>,
) -> Result<(), ()> {
    gui.borrow_mut()
        .set_progress("Examining local files...", "", Some(0.0));
    ...
    all_cats.par_iter_mut().for_each(|cat| {
        ...
        if testn == progn {
            gui.lock().unwrap().set_progress(
                "Examining local files...",
                "",
                Some(testn as f32 / num_cats as f32),
            );
        }
        let meta = match std::fs::metadata(&cat.dst_path) {
            ...
            Err(x) => {
                if x.kind() != ErrorKind::NotFound && verbose {
                    gui.lock().unwrap().verbose(&format!(
...
fn calc_rate_and_eta(
    ...
    let rate = if bytes_per_second > 1000000000.0 {
        format!("Wow!")
    } else if bytes_per_second > 800000.0 {
        format!("{:.1}MB/s", bytes_per_second / 1000000.0)
    } else if bytes_per_second > 800.0 {
...
async fn perform_downloads(
    ...
    let total_cat_bytes = all_cats
        .iter()
        .fold(0, |a, x| a + if x.needs_download { x.size } else { 0 });
    ...
    for cat in all_cats.into_iter() {
        ...
        let mut response = match client.get(cat.src_url.clone()).send().await {
            ...
            Ok(x) => {
                ...
                gui.borrow_mut().do_error(
                    "Download failed",
                    &format!(
                        "Error \"{}\" while trying to download an updated file.",
                        x.status()
                    ),
                );
                return Err(());
            ...
            Err(x) => {
                ...
                gui.borrow_mut().do_error(
                    "Download failed",
                    &format!("Couldn't download an updated file. The error was:\n{}", x),
                );
                return Err(());
            }
        };
        ...
        while file_recvd_bytes <= cat.size {
            let now = Instant::now();
            let rate_and_eta = calc_rate_and_eta(
                start_time,
                now,
                total_recvd_bytes,
                total_cat_bytes,
            ...
            if patience.have_been_patient() {
                gui.borrow_mut().set_progress(
                    "Downloading updates...",
                    &rate_and_eta,
                    Some(total_recvd_bytes as f32 / total_cat_bytes as f32),
                );
            }
            match response.chunk().await {
                Err(x) => {
                    gui.borrow_mut().do_error(
                        "Download failed",
                        &format!(
                            "Error while downloading an updated file. The error was:\n{}",
                            x
                        ),
                    );
                    return Err(());
        ...
        if sum != cat.checksum || file_recvd_bytes != cat.size {
            gui.borrow_mut().do_error(
                "Update failed",
                &format!("One of the downloads was corrupted. Try running the updater again."),
            );
            return Err(());
        }
    }
...
fn perform_deletions(
    ...
    for (n, deletion) in all_deletions.into_iter().enumerate() {
        gui.borrow_mut().set_progress(
            ...
            "",
            Some(n as f32 / num_deletions as f32),
        );
        let is_dir = match std::fs::metadata(&deletion) {
            ...
            Err(x) => {
                gui.borrow_mut().do_error(
                    "Error during final deletion",
                    &format!(
                        "Unable to get the metadata for {:?}: {}",
                        &deletion, x
                    ),
                );
                return Err(());
        ...
        let result = if is_dir {
            std::fs::remove_dir_all(&deletion)
        } else {
            std::fs::remove_file(&deletion)
        };
        if let Err(x) = result {
            gui.borrow_mut().do_error(
                "Error during final deletion",
                &format!("Unable to delete {:?}: {}", &deletion, x),
            );
            return Err(());
        }
    }
...
async fn real_main(
    ...
    let mut client = reqwest::Client::builder()
        ...
        .build()
        .unwrap();
    let (mut all_cats, mut all_deletions) =
        match determine_tasks(&gui, verbose, &mut client, &target_url).await {
            Ok(x) => x,
            Err(_) => return ExitCode::FAILURE,
        };
    ...
    if perform_downloads(&gui, verbose, &mut client, all_cats)
        .await
        .is_err()
    {
        return ExitCode::FAILURE;
    ...
    gui.borrow_mut()
        .do_message("Update complete", "All files are now up to date.");
    ExitCode::SUCCESS
}
