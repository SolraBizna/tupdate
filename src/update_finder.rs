use mlua::{
    FromLua, Function, Lua, MultiValue, Table, ThreadStatus, Value::Nil,
};
use std::{
    cell::RefCell,
    ...
    let glob = match Glob::new(&glob) {
        ...
        Err(_) => {
            return Err(mlua::Error::RuntimeError(format!(
                "Syntactically invalid glob among dir sense globs"
            )));
        }
    };
...
trait UpdateFinderRef {
    ...
    fn delete_unmatched(&self, _lua: &Lua, target: String)
        -> mlua::Result<()>;
}
...
impl UpdateFinderRef for Rc<RefCell<UpdateFinder>> {
    fn refmut(&self) -> mlua::Result<std::cell::RefMut<UpdateFinder>> {
        Ok(self
            .try_borrow_mut()
            .expect("Attempt to perform unsafe borrow on UpdateFinder"))
    }
    fn refconst(&self) -> mlua::Result<std::cell::Ref<UpdateFinder>> {
        Ok(self
            .try_borrow()
    ...
    fn check_detected_dir(
        ...
        if !candidate.is_absolute() {
            return Err(mlua::Error::RuntimeError(format!(
                "Path is invalid (must be absolute)"
            )));
        }
        let mut ok = true;
        if let Ok(globs) = silhouette.get::<_, Vec<String>>("sense") {
            for srcglob in globs.iter() {
                if !sense(candidate, srcglob)? {
                    if verbose {
                        self.refconst()?.gui.borrow_mut().verbose(&format!(
                            "    Rejected: doesn't match glob {:?}",
                            srcglob
                        ));
                    }
                    ok = false;
        ...
        if ok {
            if verbose {
                self.refconst()?
                    .gui
                    .borrow_mut()
                    .verbose(&format!("    Accepted!"));
            }
            self.refmut()?
                .dirs
                .insert(var.to_string(), candidate.to_path_buf());
        }
        Ok(ok)
    }
    fn detect_dir(
        ...
        if verbose {
            self.refconst()?
                .gui
                .borrow_mut()
                .verbose(&format!("Detecting {:?} ({}):", id, name));
        }
        if let Some(wo) = env::var_os(&id) {
            if verbose {
                self.refconst()?
                    .gui
                    .borrow_mut()
                    .verbose(&format!("  Environment variable: {:?}", wo));
            }
            if self.check_detected_dir(&id, &Path::new(&wo), &silhouette)? {
                return Ok(());
            }
        }
                ...
                Some(wo) => {
                    if verbose {
                        self.refconst()?
                            .gui
                            .borrow_mut()
                            .verbose(&format!("  Index suggests: {:?}", wo));
                    }
                    if self.check_detected_dir(
                        &id,
                        &Path::new(&wo),
                        &silhouette,
                    )? {
    ...
    fn basedir(&self, _lua: &Lua, target: String) -> mlua::Result<()> {
        ...
        if self.refconst()?.verbose {
            self.refconst()?
                .gui
                .borrow_mut()
                .verbose(&format!("Entering {:?} ({})", dir, target));
        }
    ...
    fn cd(&self, _lua: &Lua, target: String) -> mlua::Result<()> {
        if is_fishy_path(&target) {
            return Err(mlua::Error::RuntimeError(format!(
                "You cannot cd to an absolute path, or use any path component that starts with a ."
            )));
        }
        ...
        if let Some(basedir) = me.basedir.as_mut() {
            ...
            return Err(mlua::Error::RuntimeError(format!(
                "You must use basedir before you can cd"
            )));
        }
        if me.verbose {
            me.gui.borrow_mut().verbose(&format!(
                "Entering {:?}",
                me.basedir.as_ref().unwrap()
            ));
        }
        Ok(())
    }
    ...
    fn install(&self, _lua: &Lua, target: String) -> mlua::Result<()> {
        ...
        let url = me.url.join(&target).map_err(|_| {
            mlua::Error::RuntimeError(format!(
                "Install parameter must be a valid URL"
            ))
        })?;
        let basedir = if let Some(basedir) = me.basedir.as_ref() {
            basedir.clone()
        } else {
            return Err(mlua::Error::RuntimeError(format!(
                "You must call basedir before install"
            )));
        };
    ...
    fn delete_unmatched(
        ...
        if target.ends_with("/") {
            return Err(mlua::Error::RuntimeError(format!(
                "A glob ending in \"/\" is not allowed here."
            )));
        }
        ...
        if glob.has_root() || glob.has_semantic_literals() {
            return Err(mlua::Error::RuntimeError(format!(
                "Rooted globs, and semantic components (such as \"..\"), are not allowed"
            )));
        }
        let mut me = self.refmut()?;
        let basedir = if let Some(basedir) = me.basedir.as_ref() {
        ...
        match me.deletes.entry(basedir) {
            HashMapEntry::Occupied(mut ent) => {
                ent.get_mut().push(target);
            }
            HashMapEntry::Vacant(ent) => {
                ent.insert(vec![target]);
            }
        }
        Ok(())
    }
...
pub fn find_updates(
    ...
    body: &[u8],
    url: Url,
) -> Result<(Vec<(PathBuf, Url)>, HashMap<PathBuf, Vec<String>>), ()> {
    const UNSAFE_FUNCTIONS: &[&str] = &["dofile", "loadfile"];
    let lua = match mlua::Lua::new_with(
        ...
        Err(x) => {
            gui.borrow_mut().do_error(
                "Internal error",
                &format!("Unable to initialize Lua. The error was:\n{}", x),
            );
            return Err(());
        }
    };
    ...
    if cfg!(windows) {
        lua.globals().set("windows", true).unwrap();
    }
    if cfg!(unix) {
        lua.globals().set("unix", true).unwrap();
    }
    if cfg!(target_os = "macos") {
        lua.globals().set("macos", true).unwrap();
    }
    lua.globals().set("target_os", cfg!(target_os)).unwrap();
    lua.globals()
        .set("target_family", cfg!(target_family))
        .unwrap();
    lua.globals()
        .set("tupdate_version", env!("CARGO_PKG_VERSION"))
        .unwrap();
    let uf =
        Rc::new(RefCell::new(UpdateFinder::new(gui.clone(), verbose, url)));
    if verbose {
        let gui = gui.clone();
        lua.globals()
            .set(
                "print",
                lua.create_function_mut(move |lua, things: MultiValue| {
                    gui.borrow_mut().verbose(
                        &things
                            .into_iter()
                            .map(|x| String::from_lua(x, lua))
        ...
        lua.globals()
            .set(
                ...
                lua.create_function_mut(
                    move |_lua, _things: MultiValue| -> Result<_, _> {
                        Ok(())
                    },
                )
                .unwrap(),
    ...
    lua.globals()
        .set(
            "getenv",
            lua.create_function_mut(move |_lua, env: String| {
        ...
        .unwrap();
    {
        ...
        lua.globals()
            .set(
                "detect_dir",
                lua.create_function_mut(
                    move |lua, param: (String, String, Function, Table)| {
    ...
    }
    {
        ...
        lua.globals()
            .set(
                "basedir",
                lua.create_function_mut(move |lua, param: String| {
    ...
    }
    {
        ...
        lua.globals()
            .set(
                "do_warning",
                lua.create_function_mut(
                    move |_lua, param: (String, String, Option<bool>)| {
                        Ok(gui.borrow_mut().do_warning(
                            &param.0,
                            &param.1,
                            param.2.unwrap_or(false),
                        ))
                    },
                )
                .unwrap(),
        ...
        Err(x) => {
            if let mlua::Error::CallbackError { cause, .. } = x {
                ...
                gui.borrow_mut().do_error(
                    "Lua error",
                    &format!(
                        "An error occurred while processing the update index. The error was:\n{}",
                        x
                    ),
                );
            }
